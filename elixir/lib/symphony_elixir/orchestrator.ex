defmodule SymphonyElixir.Orchestrator do
  ...
  def update_issue_state(server \\ __MODULE__, issue_id, state_name)
    ...
    GenServer.call(server, {:update_issue_state, issue_id, state_name})
  end
  ...
  defp log_dispatch_error(:missing_linear_project_slug) do
    Logger.error("Linear project slug missing in WORKFLOW.md")
  end

  defp log_dispatch_error(:missing_tracker_kind) do
  ...
  defp log_dispatch_error({:invalid_codex_approval_policy, value}) do
    Logger.error("Invalid codex.approval_policy in WORKFLOW.md: #{inspect(value)}")
  end

  defp log_dispatch_error({:invalid_codex_thread_sandbox, value}) do
    Logger.error("Invalid codex.thread_sandbox in WORKFLOW.md: #{inspect(value)}")
  end

  defp log_dispatch_error({:invalid_codex_turn_sandbox_policy, reason}) do
    Logger.error("Invalid codex.turn_sandbox_policy in WORKFLOW.md: #{inspect(reason)}")
  end

  defp log_dispatch_error({:missing_workflow_file, path, reason}) do
    Logger.error("Missing WORKFLOW.md at #{path}: #{inspect(reason)}")
  end

  defp log_dispatch_error(:workflow_front_matter_not_a_map) do
    Logger.error("Failed to parse WORKFLOW.md: workflow front matter must decode to a map")
  end

  defp log_dispatch_error({:workflow_parse_error, reason}) do
    Logger.error("Failed to parse WORKFLOW.md: #{inspect(reason)}")
  end

  defp log_dispatch_error(reason) do
  ...
  def should_dispatch_issue_for_test(%Issue{} = issue, %State{} = state) do
    active_states =
      if MapSet.size(state.active_states) > 0, do: state.active_states, else: active_state_set()

    terminal_states =
      if MapSet.size(state.terminal_states) > 0,
        do: state.terminal_states,
        else: terminal_state_set()
    ...
    case Map.fetch(refreshed_issues_map, issue.id) do
      {:ok, %Issue{} = refreshed_issue} ->
        if retry_candidate_issue?(refreshed_issue, active_states, terminal_states) do
          do_dispatch_issue(state_acc, refreshed_issue, nil, nil)
        else
    ...
    case Tracker.fetch_issue_states_by_ids(candidate_ids) do
      {:ok, refreshed_issues} ->
        ...
        Enum.reduce(candidates, state, fn issue, state_acc ->
          dispatch_refreshed_candidate(
            issue,
            state_acc,
            refreshed_issues_map,
            terminal_states
          )
        end)
  ...
  defp should_dispatch_issue?(_issue, _state, _running_counts, _active_states, _terminal_states),
    do: false

  defp state_slots_available?(%Issue{state: issue_state}, running_counts)
  ...
  defp dispatch_refreshed_candidate(issue, state, refreshed_issues_map, terminal_states) do
    case Map.fetch(refreshed_issues_map, issue.id) do
      {:ok, %Issue{} = refreshed_issue} ->
        maybe_dispatch_refreshed_issue(state, refreshed_issue, terminal_states)
  ...
  defp maybe_dispatch_refreshed_issue(state, refreshed_issue, terminal_states) do
    if retry_candidate_issue?(refreshed_issue, state.active_states, terminal_states) do
      do_dispatch_issue(state, refreshed_issue, nil, nil)
    else
      Logger.info("Skipping stale dispatch after issue refresh: #{issue_context(refreshed_issue)} state=#{inspect(refreshed_issue.state)} blocked_by=#{length(refreshed_issue.blocked_by)}")
