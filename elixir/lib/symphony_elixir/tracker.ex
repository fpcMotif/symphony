defmodule SymphonyElixir.Tracker do
  ...
  def fetch_candidate_issues do
    adapter()
    |> apply(:fetch_candidate_issues, [])
    |> normalize_list_response(:fetch_candidate_issues)
  end
  ...
  def fetch_issues_by_states(states) do
    with {:ok, normalized_states} <- normalize_string_list(states, :invalid_states) do
      adapter()
      |> apply(:fetch_issues_by_states, [normalized_states])
      |> normalize_list_response(:fetch_issues_by_states)
    end
  end
  ...
  def fetch_issue_states_by_ids(issue_ids) do
    with {:ok, normalized_ids} <- normalize_string_list(issue_ids, :invalid_issue_ids) do
      adapter()
      |> apply(:fetch_issue_states_by_ids, [normalized_ids])
      |> normalize_list_response(:fetch_issue_states_by_ids)
    end
  end
  ...
  def create_comment(issue_id, body) do
    with {:ok, normalized_issue_id} <- normalize_string(issue_id, :invalid_issue_id),
      ...
      adapter()
      |> apply(:create_comment, [normalized_issue_id, normalized_body])
      |> normalize_write_response(:create_comment)
    end
  end
  ...
  def update_issue_state(issue_id, state_name) do
    with {:ok, normalized_issue_id} <- normalize_string(issue_id, :invalid_issue_id),
      ...
      adapter()
      |> apply(:update_issue_state, [normalized_issue_id, normalized_state_name])
      |> normalize_write_response(:update_issue_state)
    end
  end
