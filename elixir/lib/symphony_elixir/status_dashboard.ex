defmodule SymphonyElixir.StatusDashboard do
  ...
  defp format_snapshot_content({:ok, %{running: running, retrying: retrying} = snapshot}, tps, terminal_columns_override) do
    running_event_width = running_event_width(terminal_columns_override)

    [
      format_ok_header(snapshot, tps),
      format_running_section(running, running_event_width),
      format_backoff_section(retrying),
      closing_border()
    ]
    |> List.flatten()
    |> Enum.join("\n")
  ...
  defp format_ok_header(%{running: running, codex_totals: codex_totals} = snapshot, tps) do
    rate_limits = Map.get(snapshot, :rate_limits)
  ...
  defp format_running_section(running, running_event_width) do
  ...
  defp format_backoff_section(retrying) do
