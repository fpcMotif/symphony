defmodule SymphonyElixir.StatusDashboard do
  ...
  defp format_snapshot_content(snapshot_data, tps, terminal_columns_override \\ nil) do
    case snapshot_data do
      {:ok, %{running: running, retrying: retrying, codex_totals: codex_totals} = snapshot} ->
        ...
        [
          colorize("╭─ SYMPHONY STATUS", @ansi_bold),
          format_stats_lines(agent_count, max_agents, tps, codex_totals),
          colorize("│ Rate Limits: ", @ansi_bold) <> format_rate_limits(Map.get(snapshot, :rate_limits)),
  ...
  defp format_stats_lines(agent_count, max_agents, tps, codex_totals) do
