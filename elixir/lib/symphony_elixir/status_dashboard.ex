defmodule SymphonyElixir.StatusDashboard do
  ...
  defp format_snapshot_content(snapshot_data, tps, terminal_columns_override \\ nil) do
    case snapshot_data do
      {:ok, snapshot} ->
        format_successful_snapshot(snapshot, tps, terminal_columns_override)

      :error ->
        format_error_snapshot(tps)
    end
  end

  defp format_successful_snapshot(snapshot, tps, terminal_columns_override) do
    ...
    (format_snapshot_summary(running, codex_totals, rate_limits, polling, tps) ++
       [
  ...
  defp format_snapshot_summary(running, codex_totals, rate_limits, polling, tps) do
    project_link_lines = format_project_link_lines()
    project_refresh_line = format_project_refresh_line(polling)
  ...
  defp format_error_snapshot(tps) do
    [
      colorize("╭─ SYMPHONY STATUS", @ansi_bold),
