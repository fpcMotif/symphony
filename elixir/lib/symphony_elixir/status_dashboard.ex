defmodule SymphonyElixir.StatusDashboard do
  ...
  defp format_snapshot_content({:ok, snapshot}, tps, terminal_columns_override) do
    ...
    ([
       format_snapshot_header(snapshot, tps, running, codex_totals),
       format_snapshot_running_queue(running, running_event_width),
       format_snapshot_backoff_queue(retrying)
     ] ++ [closing_border()])
    |> List.flatten()
    |> Enum.join("\n")
  end
  ...
  defp format_snapshot_header(snapshot, tps, running, codex_totals) do
    rate_limits = Map.get(snapshot, :rate_limits)
  ...
  defp format_snapshot_running_queue(running, running_event_width) do
    ...
    ] ++ running_rows ++ running_to_backoff_spacer
  end

  defp format_snapshot_backoff_queue(retrying) do
    backoff_rows = format_retry_rows(retrying)

    [colorize("├─ Backoff queue", @ansi_bold), "│"] ++ backoff_rows
  end
