defmodule SymphonyElixir.OrchestrationE2ETest.FakeLinearClient do
  ...
  def reset!(opts) do
    ...
    issues_by_id =
      ...
      |> Enum.into(%{}, fn {id, issue} -> {id, normalize_issue(issue)} end)
...
defmodule SymphonyElixir.OrchestrationE2ETest do
  ...
  test "happy path orchestrates a full cycle and leaves an isolated workspace" do
    ...
    try do
      ...
      SymphonyElixir.OrchestrationE2ETest.FakeLinearClient.reset!(
        recipient: self(),
        candidate_responses: [{:ok, [issue]}, {:ok, []}],
        issues_by_id: %{issue.id => %{issue | state: "Done"}},
        terminal_issues: []
      )
      ...
      Application.put_env(
        ...
        SymphonyElixir.OrchestrationE2ETest.FakeLinearClient
      )
      ...
      assert_receive {:fake_linear_fetch_candidate_issues, 1, {:ok, [_issue]}}, 1_000
      ...
      assert SymphonyElixir.OrchestrationE2ETest.FakeLinearClient.candidate_call_count() >= 2
  ...
  test "transient tracker failure retries with backoff and eventually recovers" do
    ...
    try do
      ...
      assert SymphonyElixir.OrchestrationE2ETest.FakeLinearClient.candidate_call_count() == 3
    after
      File.rm_rf(test_root)
    end
  end
  ...
  defp assert_eventually(fun, timeout_ms \\ 2_000)
